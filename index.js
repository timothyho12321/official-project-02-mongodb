...
async function main() {
    ...
    app.put('/car/:car_id', validation(carPostSchema), async function (req, res) {
        ...
        // Need to manually validate input that is a nested object
        if (req.body.comments.username === "") {
            // console.log("Detected comment username is empty string")

            res.status(400)
            res.json({
                "message": "Need to provide a commenter username."
            })
            return;

        }

        if (req.body.comments.email === "") {
            // console.log("Detected comment email is empty string")

            res.status(400)
            res.json({
                "message": "Need to provide a commenter email."
            })
            return;

        }
        if (req.body.comments.other_comment === "") {
            // console.log("Detected comment description is empty string")

            res.status(400)
            res.json({
                "message": "Need to provide a comment description."
            })
            return;
            ...
            try {
                console.log(req.body.comments.username)
